...
type PreviewKind = "image" | "pdf" | "html" | "markdown" | "csv" | "json" | "text" | "binary" | "contact" | "calendar" | "email" | "link";
type NoticeTone = "info" | "success" | "error";
type WorkspaceMode = "browse" | "preview" | "edit";
...
function fileKey(file: LiteFile): string {
  return `${file.serviceId}:${file.path}`;
}

function selectedServiceFromHash(): string {
  const params = new URLSearchParams(window.location.hash.replace(/^#/, ""));
  return params.get("service") ?? "";
}

function syncHash(serviceId: string): void {
...
function serviceQuery(serviceId: string, showHidden: boolean): string {
...
function fileURL(serviceId: string, path: string): string {
...
function previewKind(file: LiteFile, contentType: string): PreviewKind {
  const ext = file.extension.toLowerCase();
  if (ext === "vcf") return "contact";
  if (ext === "ics") return "calendar";
  if (ext === "eml") return "email";
  if (ext === "webloc" || ext === "url") return "link";
  if (contentType.startsWith("image/") || imageExtensions.has(ext)) return "image";
  if (ext === "pdf" || contentType === "application/pdf") return "pdf";
  if (ext === "html" || ext === "htm") return "html";
  if (ext === "md" || ext === "markdown") return "markdown";
  if (ext === "csv") return "csv";
  if (ext === "json") return "json";
  if (textExtensions.has(ext) || contentType.startsWith("text/")) return "text";
  return "binary";
}

function filteredFiles(state: AppState): LiteFile[] {
...
function activeFile(state: AppState): LiteFile | undefined {
  return state.files.find((file) => fileKey(file) === state.activeKey);
}

function currentDirectory(file?: LiteFile): string {
  if (!file?.path.includes("/")) return "";
  return file.path.slice(0, file.path.lastIndexOf("/") + 1);
}

function currentWriteService(state: AppState): LiteService | undefined {
...
function canEditFile(file: LiteFile | undefined, preview: LoadedPreview | null): boolean {
  return Boolean(file?.editable && preview?.text !== undefined);
}

function effectiveWorkspaceMode(state: AppState): WorkspaceMode {
...
function revokePreview(preview: LoadedPreview | null): void {
  if (preview?.objectURL) {
    URL.revokeObjectURL(preview.objectURL);
  }
}

function editorLabel(kind: PreviewKind): string {
  switch (kind) {
  case "json":
    return "JSON Source";
  case "markdown":
...
function renderSummaryPairs(pairs: Array<[string, string]>): string {
...
function renderCsvPreview(text: string): string {
...
function unfoldStructuredLines(text: string): string[] {
...
function parseStructuredMap(text: string): Record<string, string[]> {
...
function parseEmail(text: string): { headers: Record<string, string>; body: string } {
...
function parseLinkURL(text: string): string {
...
function renderJsonPanel(text: string): string {
  try {
...
function renderContactPanel(text: string): string {
...
function renderCalendarPanel(text: string): string {
  const map = parseStructuredMap(text);
  return `
    <div class="section-stack">
      ${renderSummaryPairs([
...
function renderEmailPanel(text: string): string {
...
function renderLinkPanel(text: string): string {
...
function renderSourcePane(file: LiteFile, preview: LoadedPreview): string {
...
function renderSourceOnlyPane(file: LiteFile, preview: LoadedPreview): string {
  return `
...
function renderReviewPane(file: LiteFile, preview: LoadedPreview): string {
  const text = preview.text ?? "";
...
function renderVisualPane(file: LiteFile, preview: LoadedPreview): string {
  switch (preview.kind) {
  case "image":
    return `
...
function renderPreviewOnlyPane(file: LiteFile, preview: LoadedPreview): string {
  switch (preview.kind) {
  case "image":
  case "pdf":
  case "binary":
...
async function ensurePreview(state: AppState): Promise<void> {
  const file = activeFile(state);
  if (!file) {
...
function setNotice(state: AppState, tone: NoticeTone, message: string): void {
  state.noticeTone = tone;
  state.notice = message;
}

async function withBusy(state: AppState, message: string, work: () => Promise<void>): Promise<void> {
...
async function refresh(state: AppState, keepSelection = true): Promise<void> {
  const services = await fetchJSON<LiteService[]>("/lite/api/services");
  ...
  const nextFiles = await fetchJSON<LiteFile[]>(serviceQuery(state.selectedService, state.showHidden));
...
function renderNotice(state: AppState): string {
  if (!state.notice && !state.busyMessage) return "";
...
function renderServiceRail(state: AppState): string {
  const serviceItems = [
    `
...
function renderFileBrowser(state: AppState): string {
...
function renderDetail(state: AppState): string {
  const file = activeFile(state);
  if (!file) return renderOverview(state);
...
async function createEmptyFile(state: AppState): Promise<void> {
  const service = currentWriteService(state);
  if (!service) {
...
async function createFolder(state: AppState): Promise<void> {
  const service = currentWriteService(state);
  if (!service) {
...
async function uploadFile(state: AppState, file: File): Promise<void> {
  const service = currentWriteService(state);
  if (!service) {
...
function formattedEditorText(state: AppState): string {
  const editor = state.root.querySelector<HTMLTextAreaElement>("[data-editor]");
  if (!editor) {
...
async function saveActiveFile(state: AppState): Promise<void> {
  const file = activeFile(state);
...
function formatActiveFile(state: AppState): void {
  const file = activeFile(state);
  if (!file || state.preview?.kind !== "json") return;
...
async function renameActiveFile(state: AppState, file: LiteFile): Promise<void> {
...
async function deleteActiveFile(state: AppState, file: LiteFile): Promise<void> {
...
function bindEvents(state: AppState): void {
  state.root.addEventListener("click", (event) => {
    ...
    const source = target.closest<HTMLElement>("[data-action]");
    const action = source?.dataset.action;
    const serviceId = source?.dataset.serviceId ?? "";
    const path = source?.dataset.path ?? "";
    const requestedMode = source?.dataset.mode as WorkspaceMode | undefined;

    if (!action) return;

    const run = (message: string, work: () => Promise<void>) => {
    ...
    if (action === "refresh") {
      ...
      state.root.querySelector<HTMLInputElement>("#upload-input")?.click();
